...
def risk_assessment_plot(X_binarized_norm, y, ROC_models):
    ...
    probabilities = me.best_models_ROC_curves(X_binarized_norm, y, ROC_models, False)
    lr_probabilities, rf_probabilities, svm_probabilities, mlp_probabilities = probabilities

    # for survival problem
    output_list1 = me.risk_assessment_data(rf_probabilities, lr_probabilities, y)
    output_list3 = me.risk_assessment_data(svm_probabilities, mlp_probabilities, y)

    import rpy2.robjects.numpy2ri as rpyn
    rpyn.activate()

    [xrf_sens, yrf_sens, xrf_spec, yrf_spec, xlr_sens, ylr_sens, xlr_spec, ylr_spec] = rpyn.ri2py(output_list1)
    [xsvm_sens, ysvm_sens, xsvm_spec, ysvm_spec, xmlp_sens, ymlp_sens, xmlp_spec, ymlp_spec] = rpyn.ri2py(
        output_list3)

    plt.figure()
    plt.rcParams["figure.figsize"] = (10, 6)
    # for survival problem
    plt.plot(xlr_sens, ylr_sens, color='g',
             label='Events LR',
             lw=2, alpha=.8)
    plt.plot(xlr_spec, ylr_spec, color='g', linestyle='--',
             label='Non-events LR',
             lw=2, alpha=.8)
    plt.plot(xrf_sens, yrf_sens, color='b',
             label='Events RF',
             lw=2, alpha=.8)
    plt.plot(xrf_spec, yrf_spec, color='b', linestyle='--',
             label='Non-events RF',
             lw=2, alpha=.8)
    plt.plot(xsvm_sens, ysvm_sens, color='k',
             label='Events SVM',
             lw=2, alpha=.8)
    plt.plot(xsvm_spec, ysvm_spec, color='k', linestyle='--',
             label='Non-events SVM',
             lw=2, alpha=.8)
    plt.plot(xmlp_sens, ymlp_sens, color='magenta',
             label='Events MLP',
             lw=2, alpha=.8)
    plt.plot(xmlp_spec, ymlp_spec, color='magenta', linestyle='--',
             label='Non-events MLP',
             lw=2, alpha=.8)
...
def nri_idi(X_binarized_norm, y, ROC_models, pathfile):
    """
    ...
    :return:
        Save calculated values in csv files.
    """
    # for survival problem
    lr_probabilities, rf_probabilities, svm_probabilities, mlp_probabilities = me.best_models_ROC_curves(
        X_binarized_norm, y,
        ROC_models, False)
    ...
    output_list3 = me.reclassification_metrics(mlp_probabilities, svm_probabilities, y, t=0.245)

    rf_mlp_filename = pathfile + 'RF_MLP.csv'
    lr_mlp_filename = pathfile + 'LR_MLP.csv'
    mlp_svm_filename = pathfile + 'MLP_SVM.csv'

    write_file = ro.r['write.csv']

    write_file(output_list1, rf_mlp_filename)
    write_file(output_list2, lr_mlp_filename)
    write_file(output_list3, mlp_svm_filename)
...
filename = 'Dataset/survival_minor class is event.csv'
X_binarized_norm, y = loadFile(filename, [13], binarized=True, norm=True)
...
models = [{'name': 'lr', 'label': 'Logistic Regression',
          ...
          # hyperparameter tunning cannot be performed for SVM with RFE algorithm for feature selection
          ...
          # hyperparameter tunning cannot be performed for MLP with RFE algorithm for feature selection
...
ROC_models = [{"name": "LR_RFE_30",
              ...
              {"name": 'RF_ReliefF_50',
               "classifier": RandomForestClassifier(random_state=88),
               "grid": {'classifier__n_estimators': [100, 200]},
               "fs_method": ReliefF(n_neighbors=10, n_features_to_keep=50),
               "color_line": "b",
               "color_shadow": "lightblue"},

              {"name": "SVM_mRMR_50",
               "classifier": SVC(random_state=88, C=1, gamma=0.001, probability=True),
               "grid": {},
               "fs_method": MRMR(n_features=50),
               "color_line": 'black',
               "color_shadow": 'silver'},
